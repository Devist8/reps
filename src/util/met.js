const met = {
    bicycling: [
        {
            activity: "bicycling",
            motion: "bicycling, mountain, uphill, vigorous",
            met: "14.0",
        },
        {
            activity: "bicycling",
            motion: "bicycling, mountain, competitive, racing",
            met: "16.0",
        },
        {
            activity: "bicycling",
            motion: "bicycling, BMX",
            met: "8.5",
        },
        {
            activity: "bicycling",
            motion: "bicycling, mountain, general",
            met: "8.5",
        },
        {
            activity: "bicycling",
            motion: "bicycling, <10 mph, leisure, to work or for pleasure",
            met: "4.0",
        },
        {
            activity: "bicycling",
            motion: "bicycling, to/from work, self selected pace",
            met: "6.8",
        },
        {
            activity: "bicycling",
            motion: "bicycling, on dirt or farm road, moderate pace",
            met: "5.8",
        },
        {
            activity: "bicycling",
            motion: "bicycling, general",
            met: "7.5",
        },
        {
            activity: "bicycling",
            motion: "bicycling, leisure, 5.5 mph",
            met: "3.5",
        },
        {
            activity: "bicycling",
            motion: "bicycling, leisure, 9.4 mph",
            met: "5.8",
        },
        {
            activity: "bicycling",
            motion: "bicycling, 10-11.9 mph, leisure, slow, light effort",
            met: "6.8",
        },
        {
            activity: "bicycling",
            motion: "bicycling, 12-13.9 mph, leisure, moderate effort",
            met: "8.0",
        },
        {
            activity: "bicycling",
            motion: "bicycling, 14-15.9 mph, racing or leisure, fast, vigorous effort",
            met: "10.0",
        },
        {
            activity: "bicycling",
            motion: "bicycling, 16-19 mph, racing/not drafting or > 19 mph drafting, very fast, racing general",
            met: "12.0",
        },
        {
            activity: "bicycling",
            motion: "bicycling, > 20 mph, racing, not drafting",
            met: "15.8",
        },
        {
            activity: "bicycling",
            motion: "bicycling, 12 mph, seated, hands on brake hoods or bar drops, 80 rpm",
            met: "8.5",
        },
        {
            activity: "bicycling",
            motion: "bicycling, 12 mph, standing, hands on brake hoods, 60 rpm",
            met: "9.0",
        },
        {
            activity: "bicycling",
            motion: "unicycling",
            met: "5.0",
        },
    ],
    calisthenics: [
        {
            activity: "calisthenics",
            motion: "push ups, vigorous effort",
            met: "8.0",
        },
        {
            activity: "calisthenics",
            motion: "pull-ups, vigorous effort",
            met: "8.0",
        },
        {
            activity: "calisthenics",
            motion: "lunges, vigorous effort",
            met: "8.0",
        },
        {
            activity: "calisthenics",
            motion: "crunches, vigorous effort",
            met: "8.0",
        },
        {
            activity: "calisthenics",
            motion: "squats, vigorous effort",
            met: "8.0",
        },
        {
            activity: "calisthenics",
            motion: "push ups, moderate effort",
            met: "3.8",
        },
        {
            activity: "calisthenics",
            motion: "pull ups, moderate effort",
            met: "3.8",
        },
        {
            activity: "calisthenics",
            motion: "lunges, moderate effort",
            met: "3.8",
        },
        {
            activity: "calisthenics",
            motion: "squats, moderate effort",
            met: "3.8",
        },
        {
            activity: "calisthenics",
            motion: "sit ups, light effort",
            met: "2.8",
        },
        {
            activity: "calisthenics",
            motion: "crunches, light effort",
            met: "2.8",
        },
        {
            activity: "calisthenics",
            motion: "planks, light effort",
            met: "2.8",
        },
        {
            activity: "calisthenics",
            motion: "lunges, light effort",
            met: "2.8",
        },
        {
            activity: "calisthenics",
            motion: "squats, light effort",
            met: "2.8",
        },
    ],
    conditioning: [
        {
            activity: "conditioning exercise",
            motion: "activity promoting video game (e.g., Wii Fit), light effort (e.g., balance, yoga)",
            met: "2.3",
        },
        {
            activity: "conditioning exercise",
            motion: "activity promoting video game (e.g., Wii Fit), moderate effort (e.g., aerobic, resistance)",
            met: "3.8",
        },
        {
            activity: "conditioning exercise",
            motion: "activity promoting video/arcade game (e.g., Exergaming, Dance Dance Revolution), vigorous effort",
            met: "7.2",
        },
        {
            activity: "conditioning exercise",
            motion: "army type obstacle course exercise, boot camp training program",
            met: "5.0",
        },
        {
            activity: "conditioning exercise",
            motion: "circuit training, moderate effort",
            met: "4.3",
        },
        {
            activity: "conditioning exercise",
            motion: "circuit training, including kettlebells, some aerobic movement with minimal rest, general, vigorous intensity",
            met: "8.0",
        },
        {
            activity: "conditioning exercise",
            motion: "CurvesTM exercise routines in women",
            met: "3.5",
        },
        {
            activity: "conditioning exercise",
            motion: "Elliptical trainer, moderate effort",
            met: "5.0",
        },
        {
            activity: "conditioning exercise",
            motion: "resistance training (weight lifting, free weight, nautilus or universal), power lifting or body building, vigorous effort",
            met: "6.0",
        },
        {
            activity: "conditioning exercise",
            motion: "resistance (weight) training, squats , slow or metlosive effort",
            met: "5.0",
        },
        {
            activity: "conditioning exercise",
            motion: "resistance (weight) training, multiple exercises, 8-15 repetitions at varied resistance",
            met: "3.5",
        },
        {
            activity: "conditioning exercise",
            motion: "health club exercise, general",
            met: "5.5",
        },
        {
            activity: "conditioning exercise",
            motion: "health club exercise classes, general, gym/weight training combined in one visit",
            met: "5.0",
        },
        {
            activity: "conditioning exercise",
            motion: "health club exercise, conditioning classes",
            met: "7.8",
        },
        {
            activity: "conditioning exercise",
            motion: "home exercise, general",
            met: "3.8",
        },
        {
            activity: "conditioning exercise",
            motion: "stair-treadmill ergometer, general",
            met: "9.0",
        },
        {
            activity: "conditioning exercise",
            motion: "rope skipping, general",
            met: "12.3",
        },
        {
            activity: "conditioning exercise",
            motion: "rowing, stationary ergometer, general, vigorous effort",
            met: "6.0",
        },
        {
            activity: "conditioning exercise",
            motion: "rowing, stationary, general, moderate effort",
            met: "4.8",
        },
        {
            activity: "conditioning exercise",
            motion: "rowing, stationary, 100 watts, moderate effort",
            met: "7.0",
        },
        {
            activity: "conditioning exercise",
            motion: "rowing, stationary, 150 watts, vigorous effort",
            met: "8.5",
        },
        {
            activity: "conditioning exercise",
            motion: "rowing, stationary, 200 watts, very vigorous effort",
            met: "12.0",
        },
        {
            activity: "conditioning exercise",
            motion: "ski machine, general",
            met: "6.8",
        },
        {
            activity: "conditioning exercise",
            motion: "slide board exercise, general",
            met: "11.0",
        },
        {
            activity: "conditioning exercise",
            motion: "slimnastics, jazzercise",
            met: "6.0",
        },
        {
            activity: "conditioning exercise",
            motion: "stretching, mild",
            met: "2.3",
        },
        {
            activity: "conditioning exercise",
            motion: "pilates, general",
            met: "3.0",
        },
        {
            activity: "conditioning exercise",
            motion: "teaching exercise class (e.g., aerobic, water)",
            met: "6.8",
        },
        {
            activity: "conditioning exercise",
            motion: "therapeutic exercise ball, Fitball exercise",
            met: "2.8",
        },
        {
            activity: "conditioning exercise",
            motion: "upper body exercise, arm ergometer",
            met: "2.8",
        },
        {
            activity: "conditioning exercise",
            motion: "lower body exercise, light effort",
            met: "2.8",
        },
        {
            activity: "conditioning exercise",
            motion: "lower body exercise, moderate effort",
            met: "3.8",
        },
        {
            activity: "conditioning exercise",
            motion: "upper body exercise, stationary bicycle – Airdyne (arms only) 40 rpm, moderate",
            met: "4.3",
        },
        {
            activity: "conditioning exercise",
            motion: "water aerobics, water calisthenics, water exercise",
            met: "5.3",
        },
        {
            activity: "conditioning exercise",
            motion: "whirlpool, sitting",
            met: "1.3",
        },
        {
            activity: "conditioning exercise",
            motion: "video exercise workouts, TV conditioning programs (e.g., yoga, stretching), light effort",
            met: "2.3",
        },
        {
            activity: "conditioning exercise",
            motion: "video exercise workouts, TV conditioning programs (e.g., cardio-resistance), moderate effort",
            met: "4.0",
        },
        {
            activity: "conditioning exercise",
            motion: "video exercise workouts, TV conditioning programs (e.g., cardio-resistance), vigorous effort",
            met: "6.0",
        },
        {
            activity: "conditioning exercise",
            motion: "yoga, Hatha",
            met: "2.5",
        },
        {
            activity: "conditioning exercise",
            motion: "yoga, Power",
            met: "4.0",
        },
        {
            activity: "conditioning exercise",
            motion: "yoga, Nadisodhana",
            met: "2.0",
        },
        {
            activity: "conditioning exercise",
            motion: "yoga, Surya Namaskar",
            met: "3.3",
        },
        {
            activity: "conditioning exercise",
            motion: "native New Zealander physical activities (e.g., Haka Powhiri, Moteatea, Waita Tira, Whakawatea, etc.), general, moderate effort",
            met: "5.3",
        },
        {
            activity: "conditioning exercise",
            motion: "native New Zealander physical activities (e.g., Haka, Taiahab), general, vigorous effort",
            met: "6.8",
        },
    ],
    dancing: [
        {
            activity: "dancing",
            motion: "ballet, modern, or jazz, general, rehearsal or class",
            met: "5.0",
        },
        {
            activity: "dancing",
            motion: "ballet, modern, or jazz, performance, vigorous effort",
            met: "6.8",
        },
        {
            activity: "dancing",
            motion: "tap",
            met: "4.8",
        },
        {
            activity: "dancing",
            motion: "aerobic, general",
            met: "7.3",
        },
        {
            activity: "dancing",
            motion: "aerobic, step, with 6 – 8 inch step",
            met: "7.5",
        },
        {
            activity: "dancing",
            motion: "aerobic, step, with 10 – 12 inch step",
            met: "9.5",
        },
        {
            activity: "dancing",
            motion: "aerobic, step, with 4-inch step",
            met: "5.5",
        },
        {
            activity: "dancing",
            motion: "bench step class, general",
            met: "8.5",
        },
        {
            activity: "dancing",
            motion: "aerobic, low impact",
            met: "5.0",
        },
        {
            activity: "dancing",
            motion: "aerobic, high impact",
            met: "7.3",
        },
        {
            activity: "dancing",
            motion: "aerobic dance wearing 10-15 lb weights",
            met: "10.0",
        },
        {
            activity: "dancing",
            motion: "ethnic or cultural dancing (e.g., Greek, Middle Eastern, hula, salsa, merengue, bamba y plena, flamenco, belly, and swing)",
            met: "4.5",
        },
        {
            activity: "dancing",
            motion: "ballroom, fast",
            met: "5.5",
        },
        {
            activity: "dancing",
            motion: "general dancing (e.g., disco, folk, Irish step dancing, line dancing, polka, contra, country)",
            met: "7.8",
        },
        {
            activity: "dancing",
            motion: "ballroom dancing, competitive, general",
            met: "11.3",
        },
        {
            activity: "dancing",
            motion: "ballroom, slow (e.g., waltz, foxtrot, slow dancing, samba, tango, 19th century dance, mambo, cha cha)",
            met: "3.0",
        },
        {
            activity: "dancing",
            motion: "Anishinaabe Jingle Dancing",
            met: "5.5",
        },
        {
            activity: "dancing",
            motion: "Caribbean dance (Abakua, Beguine, Bellair, Bongo, Brukin’s, Caribbean Quadrills, Dinki Mini, Gere, Gumbay, Ibo, Jonkonnu, Kumina, Oreisha, Jambu)",
            met: "3.5",
        },
    ],
    hunting: [
        {
            activity: "fishing and hunting",
            motion: "fishing, general",
            met: "3.5",
        },
        {
            activity: "fishing and hunting",
            motion: "fishing, crab fishing",
            met: "4.5",
        },
        {
            activity: "fishing and hunting",
            motion: "fishing, catching fish with hands",
            met: "4.0",
        },
        {
            activity: "fishing and hunting",
            motion: "fishing related, digging worms, with shovel",
            met: "4.3",
        },
        {
            activity: "fishing and hunting",
            motion: "fishing from river bank and walking",
            met: "4.0",
        },
        {
            activity: "fishing and hunting",
            motion: "fishing from boat or canoe, sitting",
            met: "2.0",
        },
        {
            activity: "fishing and hunting",
            motion: "fishing from river bank, standing",
            met: "3.5",
        },
        {
            activity: "fishing and hunting",
            motion: "fishing in stream, in waders",
            met: "6.0",
        },
        {
            activity: "fishing and hunting",
            motion: "fishing, ice, sitting",
            met: "2.0",
        },
        {
            activity: "fishing and hunting",
            motion: "fishing, jog or line, standing, general",
            met: "1.8",
        },
        {
            activity: "fishing and hunting",
            motion: "fishing, dip net, setting net and retrieving fish, general",
            met: "3.5",
        },
        {
            activity: "fishing and hunting",
            motion: "fishing, set net, setting net and retrieving fish, general",
            met: "3.8",
        },
        {
            activity: "fishing and hunting",
            motion: "fishing, fishing wheel, setting net and retrieving fish, general",
            met: "3.0",
        },
        {
            activity: "fishing and hunting",
            motion: "fishing with a spear, standing",
            met: "2.3",
        },
        {
            activity: "fishing and hunting",
            motion: "hunting, bow and arrow, or crossbow",
            met: "2.5",
        },
        {
            activity: "fishing and hunting",
            motion: "hunting, deer, elk, large game",
            met: "6.0",
        },
        {
            activity: "fishing and hunting",
            motion: "hunting large game, dragging carcass",
            met: "11.3",
        },
        {
            activity: "fishing and hunting",
            motion: "hunting large marine animals",
            met: "4.0",
        },
        {
            activity: "fishing and hunting",
            motion: "hunting large game, from a hunting stand, limited walking",
            met: "2.5",
        },
        {
            activity: "fishing and hunting",
            motion: "hunting large game from a car, plane, or boat",
            met: "2.0",
        },
        {
            activity: "fishing and hunting",
            motion: "hunting, duck, wading",
            met: "2.5",
        },
        {
            activity: "fishing and hunting",
            motion: "hunting, flying fox, squirrel",
            met: "3.0",
        },
        {
            activity: "fishing and hunting",
            motion: "hunting, general",
            met: "5.0",
        },
        {
            activity: "fishing and hunting",
            motion: "hunting, pheasants or grouse",
            met: "6.0",
        },
        {
            activity: "fishing and hunting",
            motion: "hunting, birds",
            met: "3.3",
        },
        {
            activity: "fishing and hunting",
            motion: "hunting, rabbit, squirrel, prairie chick, raccoon, small game",
            met: "5.0",
        },
        {
            activity: "fishing and hunting",
            motion: "hunting, pigs, wild",
            met: "3.3",
        },
        {
            activity: "fishing and hunting",
            motion: "trapping game, general",
            met: "2.0",
        },
        {
            activity: "fishing and hunting",
            motion: "hunting, hiking with hunting gear",
            met: "9.5",
        },
        {
            activity: "fishing and hunting",
            motion: "pistol shooting or trap shooting, standing",
            met: "2.5",
        },
        {
            activity: "fishing and hunting",
            motion: "rifle exercises, shooting, lying down",
            met: "2.3",
        },
        {
            activity: "fishing and hunting",
            motion: "rifle exercises, shooting, kneeling or standing",
            met: "2.5",
        },
    ],

    running: [
        {
            activity: "running",
            motion: "jog/walk combination (jogging component of less than 10 minutes)",
            met: "6.0",
        },
        {
            activity: "running",
            motion: "jogging, general",
            met: "7.0",
        },
        {
            activity: "running",
            motion: "jogging, in place",
            met: "8.0",
        },
        {
            activity: "running",
            motion: "jogging, on a mini-tramp",
            met: "4.5",
        },
        {
            activity: "running",
            motion: "Running, 4 mph (13 min/mile)",
            met: "6.0",
        },
        {
            activity: "running",
            motion: "running, 5 mph (12 min/mile)",
            met: "8.3",
        },
        {
            activity: "running",
            motion: "running, 5.2 mph (11.5 min/mile)",
            met: "9.0",
        },
        {
            activity: "running",
            motion: "running, 6 mph (10 min/mile)",
            met: "9.8",
        },
        {
            activity: "running",
            motion: "running, 6.7 mph (9 min/mile)",
            met: "10.5",
        },
        {
            activity: "running",
            motion: "running, 7 mph (8.5 min/mile)",
            met: "11.0",
        },
        {
            activity: "running",
            motion: "running, 7.5 mph (8 min/mile)",
            met: "11.5",
        },
        {
            activity: "running",
            motion: "running, 8 mph (7.5 min/mile)",
            met: "11.8",
        },
        {
            activity: "running",
            motion: "running, 8.6 mph (7 min/mile)",
            met: "12.3",
        },
        {
            activity: "running",
            motion: "running, 9 mph (6.5 min/mile)",
            met: "12.8",
        },
        {
            activity: "running",
            motion: "running, 10 mph (6 min/mile)",
            met: "14.5",
        },
        {
            activity: "running",
            motion: "running, 11 mph (5.5 min/mile)",
            met: "16.0",
        },
        {
            activity: "running",
            motion: "running, 12 mph (5 min/mile)",
            met: "19.0",
        },
        {
            activity: "running",
            motion: "running, 13 mph (4.6 min/mile)",
            met: "19.8",
        },
        {
            activity: "running",
            motion: "running, 14 mph (4.3 min/mile)",
            met: "23.0",
        },
        {
            activity: "running",
            motion: "running, cross country",
            met: "9.0",
        },
        {
            activity: "running",
            motion: "running",
            met: "8.0",
        },
        {
            activity: "running",
            motion: "running, stairs, up",
            met: "15.0",
        },
        {
            activity: "running",
            motion: "running, on a track, team practice",
            met: "10.0",
        },
        {
            activity: "running",
            motion: "running, training, pushing a wheelchair or baby carrier",
            met: "8.0",
        },
        {
            activity: "running",
            motion: "running, marathon",
            met: "13.3",
        },
    ],
    sports: [
        {
            activity: "sports",
            motion: "Alaska Native Games, Eskimo Olympics, general",
            met: "5.5",
        },
        {
            activity: "sports",
            motion: "archery, non-hunting",
            met: "4.3",
        },
        {
            activity: "sports",
            motion: "badminton, competitive",
            met: "7.0",
        },
        {
            activity: "sports",
            motion: "badminton, social singles and doubles, general",
            met: "5.5",
        },
        {
            activity: "sports",
            motion: "basketball, game",
            met: "8.0",
        },
        {
            activity: "sports",
            motion: "basketball, non-game, general",
            met: "6.0",
        },
        {
            activity: "sports",
            motion: "basketball, general",
            met: "6.5",
        },
        {
            activity: "sports",
            motion: "basketball, officiating",
            met: "7.0",
        },
        {
            activity: "sports",
            motion: "basketball, shooting baskets",
            met: "4.5",
        },
        {
            activity: "sports",
            motion: "basketball, drills, practice",
            met: "9.3",
        },
        {
            activity: "sports",
            motion: "basketball, wheelchair",
            met: "7.8",
        },
        {
            activity: "sports",
            motion: "billiards",
            met: "2.5",
        },
        {
            activity: "sports",
            motion: "bowling",
            met: "3.0",
        },
        {
            activity: "sports",
            motion: "bowling, indoor, bowling alley",
            met: "3.8",
        },
        {
            activity: "sports",
            motion: "boxing, in ring, general",
            met: "12.8",
        },
        {
            activity: "sports",
            motion: "boxing, punching bag",
            met: "5.5",
        },
        {
            activity: "sports",
            motion: "boxing, sparring",
            met: "7.8",
        },
        {
            activity: "sports",
            motion: "broomball",
            met: "7.0",
        },
        {
            activity: "sports",
            motion: "children’s games, adults playing (e.g., hopscotch, 4-square, dodgeball, playground apparatus, t-ball, tetherball, marbles, arcade games), moderate effort",
            met: "5.8",
        },
        {
            activity: "sports",
            motion: "cheerleading, gymnastic moves, competitive",
            met: "6.0",
        },
        {
            activity: "sports",
            motion: "coaching, football, soccer, basketball, baseball, swimming, etc.",
            met: "4.0",
        },
        {
            activity: "sports",
            motion: "coaching, actively playing sport with players",
            met: "8.0",
        },
        {
            activity: "sports",
            motion: "cricket, batting, bowling, fielding",
            met: "4.8",
        },
        {
            activity: "sports",
            motion: "croquet",
            met: "3.3",
        },
        {
            activity: "sports",
            motion: "curling",
            met: "4.0",
        },
        {
            activity: "sports",
            motion: "darts, wall or lawn",
            met: "2.5",
        },
        {
            activity: "sports",
            motion: "drag racing, pushing or driving a car",
            met: "6.0",
        },
        {
            activity: "sports",
            motion: "auto racing, open wheel",
            met: "8.5",
        },
        {
            activity: "sports",
            motion: "fencing",
            met: "6.0",
        },
        {
            activity: "sports",
            motion: "football, competitive",
            met: "8.0",
        },
        {
            activity: "sports",
            motion: "football, touch, flag, general",
            met: "8.0",
        },
        {
            activity: "sports",
            motion: "football, touch, flag, light effort",
            met: "4.0",
        },
        {
            activity: "sports",
            motion: "football or baseball, playing catch",
            met: "2.5",
        },
        {
            activity: "sports",
            motion: "frisbee playing, general",
            met: "3.0",
        },
        {
            activity: "sports",
            motion: "frisbee, ultimate",
            met: "8.0",
        },
        {
            activity: "sports",
            motion: "golf, general",
            met: "4.8",
        },
        {
            activity: "sports",
            motion: "golf, walking, carrying clubs",
            met: "4.3",
        },
        {
            activity: "sports",
            motion: "golf, miniature, driving range",
            met: "3.0",
        },
        {
            activity: "sports",
            motion: "golf, walking, pulling clubs",
            met: "5.3",
        },
        {
            activity: "sports",
            motion: "golf, using power cart",
            met: "3.5",
        },
        {
            activity: "sports",
            motion: "gymnastics, general",
            met: "3.8",
        },
        {
            activity: "sports",
            motion: "hacky sack",
            met: "4.0",
        },
        {
            activity: "sports",
            motion: "handball, general",
            met: "12.0",
        },
        {
            activity: "sports",
            motion: "handball, team",
            met: "8.0",
        },
        {
            activity: "sports",
            motion: "high ropes course, multiple elements",
            met: "4.0",
        },
        {
            activity: "sports",
            motion: "hang gliding",
            met: "3.5",
        },
        {
            activity: "sports",
            motion: "hockey, field",
            met: "7.8",
        },
        {
            activity: "sports",
            motion: "hockey, ice, general",
            met: "8.0",
        },
        {
            activity: "sports",
            motion: "hockey, ice, competitive",
            met: "10.0",
        },
        {
            activity: "sports",
            motion: "horseback riding, general",
            met: "5.5",
        },
        {
            activity: "sports",
            motion: "horse chores, feeding, watering, cleaning stalls, implied walking and lifting loads",
            met: "4.3",
        },
        {
            activity: "sports",
            motion: "saddling, cleaning, grooming, harnessing and unharnessing horse",
            met: "4.5",
        },
        {
            activity: "sports",
            motion: "horseback riding, trotting",
            met: "5.8",
        },
        {
            activity: "sports",
            motion: "horseback riding, canter or gallop",
            met: "7.3",
        },
        {
            activity: "sports",
            motion: "horseback riding,walking",
            met: "3.8",
        },
        {
            activity: "sports",
            motion: "horseback riding, jumping",
            met: "9.0",
        },
        {
            activity: "sports",
            motion: "horse cart, driving, standing or sitting",
            met: "1.8",
        },
        {
            activity: "sports",
            motion: "horseshoe pitching, quoits",
            met: "3.0",
        },
        {
            activity: "sports",
            motion: "jai alai",
            met: "12.0",
        },
        {
            activity: "sports",
            motion: "martial arts, different types, slower pace, novice performers, practice",
            met: "5.3",
        },
        {
            activity: "sports",
            motion: "martial arts, different types, moderate pace (e.g., judo, jujitsu, karate, kick boxing, tae kwan do, tai-bo, Muay Thai boxing)",
            met: "10.3",
        },
        {
            activity: "sports",
            motion: "juggling",
            met: "4.0",
        },
        {
            activity: "sports",
            motion: "kickball",
            met: "7.0",
        },
        {
            activity: "sports",
            motion: "lacrosse",
            met: "8.0",
        },
        {
            activity: "sports",
            motion: "lawn bowling, bocce ball, outdoor",
            met: "3.3",
        },
        {
            activity: "sports",
            motion: "moto-cross, off-road motor sports, all-terrain vehicle, general",
            met: "4.0",
        },
        {
            activity: "sports",
            motion: "orienteering",
            met: "9.0",
        },
        {
            activity: "sports",
            motion: "paddleball, competitive",
            met: "10.0",
        },
        {
            activity: "sports",
            motion: "paddleball, casual, general",
            met: "6.0",
        },
        {
            activity: "sports",
            motion: "polo, on horseback",
            met: "8.0",
        },
        {
            activity: "sports",
            motion: "racquetball, competitive",
            met: "10.0",
        },
        {
            activity: "sports",
            motion: "racquetball, general",
            met: "7.0",
        },
        {
            activity: "sports",
            motion: "rock or mountain climbing",
            met: "8.0",
        },
        {
            activity: "sports",
            motion: "rock climbing, ascending rock, high difficulty",
            met: "7.5",
        },
        {
            activity: "sports",
            motion: "rock climbing, ascending or traversing rock, low-to-moderate difficulty",
            met: "5.8",
        },
        {
            activity: "sports",
            motion: "rock climbing, rappelling",
            met: "5.0",
        },
        {
            activity: "sports",
            motion: "rodeo sports, general, light effort",
            met: "4.0",
        },
        {
            activity: "sports",
            motion: "rodeo sports, general, moderate effort",
            met: "5.5",
        },
        {
            activity: "sports",
            motion: "rodeo sports, general, vigorous effort",
            met: "7.0",
        },
        {
            activity: "sports",
            motion: "rope jumping, fast pace, 120-160 skips/min",
            met: "12.3",
        },
        {
            activity: "sports",
            motion: "rope jumping, moderate pace, 100-120 skips/min, general, 2 foot skip, plain bounce",
            met: "11.8",
        },
        {
            activity: "sports",
            motion: "rope jumping, slow pace, < 100 skips/min, 2 foot skip, rhythm bounce",
            met: "8.8",
        },
        {
            activity: "sports",
            motion: "rugby, union, team, competitive",
            met: "8.3",
        },
        {
            activity: "sports",
            motion: "rugby, touch, non-competitive",
            met: "6.3",
        },
        {
            activity: "sports",
            motion: "shuffleboard",
            met: "3.0",
        },
        {
            activity: "sports",
            motion: "skateboarding, general, moderate effort",
            met: "5.0",
        },
        {
            activity: "sports",
            motion: "skateboarding, competitive, vigorous effort",
            met: "6.0",
        },
        {
            activity: "sports",
            motion: "skating, roller",
            met: "7.0",
        },
        {
            activity: "sports",
            motion: "rollerblading, in-line skating, 14.4 km/h (9.0 mph), recreational pace",
            met: "7.5",
        },
        {
            activity: "sports",
            motion: "rollerblading, in-line skating, 17.7 km/h (11.0 mph), moderate pace, exercise training",
            met: "9.8",
        },
        {
            activity: "sports",
            motion: "rollerblading, in-line skating, 21.0 to 21.7 km/h (13.0 to 13.6 mph), fast pace, exercise training",
            met: "12.3",
        },
        {
            activity: "sports",
            motion: "rollerblading, in-line skating, 24.0 km/h (15.0 mph), maximal effort",
            met: "14.0",
        },
        {
            activity: "sports",
            motion: "skydiving, base jumping, bungee jumping",
            met: "3.5",
        },
        {
            activity: "sports",
            motion: "soccer, competitive",
            met: "10.0",
        },
        {
            activity: "sports",
            motion: "soccer, casual, general",
            met: "7.0",
        },
        {
            activity: "sports",
            motion: "softball or baseball, fast or slow pitch, general",
            met: "5.0",
        },
        {
            activity: "sports",
            motion: "softball, practice",
            met: "4.0",
        },
        {
            activity: "sports",
            motion: "softball, officiating",
            met: "4.0",
        },
        {
            activity: "sports",
            motion: "softball, pitching",
            met: "6.0",
        },
        {
            activity: "sports",
            motion: "sports spectator, very excited, emotional, physically moving",
            met: "3.3",
        },
        {
            activity: "sports",
            motion: "squash",
            met: "12.0",
        },
        {
            activity: "sports",
            motion: "squash, general",
            met: "7.3",
        },
        {
            activity: "sports",
            motion: "table tennis, ping pong",
            met: "4.0",
        },
        {
            activity: "sports",
            motion: "tai chi, qi gong, general",
            met: "3.0",
        },
        {
            activity: "sports",
            motion: "tai chi, qi gong, sitting, light effort",
            met: "1.5",
        },
        {
            activity: "sports",
            motion: "tennis, general",
            met: "7.3",
        },
        {
            activity: "sports",
            motion: "tennis, doubles",
            met: "6.0",
        },
        {
            activity: "sports",
            motion: "tennis, doubles",
            met: "4.5",
        },
        {
            activity: "sports",
            motion: "tennis, singles",
            met: "8.0",
        },
        {
            activity: "sports",
            motion: "tennis, hitting balls, non-game play, moderate effort",
            met: "5.0",
        },
        {
            activity: "sports",
            motion: "trampoline, recreational",
            met: "3.5",
        },
        {
            activity: "sports",
            motion: "trampoline, competitive",
            met: "4.5",
        },
        {
            activity: "sports",
            motion: "volleyball",
            met: "4.0",
        },
        {
            activity: "sports",
            motion: "volleyball, competitive, in gymnasium",
            met: "6.0",
        },
        {
            activity: "sports",
            motion: "volleyball, non-competitive, 6 – 9 member team, general",
            met: "3.0",
        },
        {
            activity: "sports",
            motion: "volleyball, beach, in sand",
            met: "8.0",
        },
        {
            activity: "sports",
            motion: "wrestling (one match = 5 minutes)",
            met: "6.0",
        },
        {
            activity: "sports",
            motion: "wallyball, general",
            met: "7.0",
        },
        {
            activity: "sports",
            motion: "track and field (e.g., shot, discus, hammer throw)",
            met: "4.0",
        },
        {
            activity: "sports",
            motion: "track and field (e.g., high jump, long jump, triple jump, javelin, pole vault)",
            met: "6.0",
        },
        {
            activity: "sports",
            motion: "track and field (e.g., steeplechase, hurdles)",
            met: "10.0",
        },
    ],
    walking: [
        {
            activity: "walking",
            motion: "backpacking",
            met: "7.0",
        },
        {
            activity: "walking",
            motion: "backpacking, hiking or organized walking with a daypack",
            met: "7.8",
        },
        {
            activity: "walking",
            motion: "carrying 15 pound load (e.g. suitcase), level ground or downstairs",
            met: "5.0",
        },
        {
            activity: "walking",
            motion: "carrying 15 lb child, slow walking",
            met: "2.3",
        },
        {
            activity: "walking",
            motion: "carrying load upstairs, general",
            met: "8.3",
        },
        {
            activity: "walking",
            motion: "carrying 1 to 15 lb load, upstairs",
            met: "5.0",
        },
        {
            activity: "walking",
            motion: "carrying 16 to 24 lb load, upstairs",
            met: "6.0",
        },
        {
            activity: "walking",
            motion: "carrying 25 to 49 lb load, upstairs",
            met: "8.0",
        },
        {
            activity: "walking",
            motion: "carrying 50 to 74 lb load, upstairs",
            met: "10.0",
        },
        {
            activity: "walking",
            motion: "carrying > 74 lb load, upstairs",
            met: "12.0",
        },
        {
            activity: "walking",
            motion: "loading /unloading a car, implied walking",
            met: "3.5",
        },
        {
            activity: "walking",
            motion: "climbing hills, no load",
            met: "6.3",
        },
        {
            activity: "walking",
            motion: "climbing hills with 0 to 9 lb load",
            met: "6.5",
        },
        {
            activity: "walking",
            motion: "climbing hills with 10 to 20 lb load",
            met: "7.3",
        },
        {
            activity: "walking",
            motion: "climbing hills with 21 to 42 lb load",
            met: "8.3",
        },
        {
            activity: "walking",
            motion: "climbing hills with 42+ lb load",
            met: "9.0",
        },
        {
            activity: "walking",
            motion: "descending stairs",
            met: "3.5",
        },
        {
            activity: "walking",
            motion: "hiking, cross country",
            met: "6.0",
        },
        {
            activity: "walking",
            motion: "hiking or walking at a normal pace through fields and hillsides",
            met: "5.3",
        },
        {
            activity: "walking",
            motion: "bird watching, slow walk",
            met: "2.5",
        },
        {
            activity: "walking",
            motion: "marching, moderate speed, military, no pack",
            met: "4.5",
        },
        {
            activity: "walking",
            motion: "marching rapidly, military, no pack",
            met: "8.0",
        },
        {
            activity: "walking",
            motion: "pushing or pulling stroller with child or walking with children, 2.5 to 3.1 mph",
            met: "4.0",
        },
        {
            activity: "walking",
            motion: "pushing a wheelchair, non-occupational",
            met: "3.8",
        },
        {
            activity: "walking",
            motion: "race walking",
            met: "6.5",
        },
        {
            activity: "walking",
            motion: "stair climbing, using or climbing up ladder",
            met: "8.0",
        },
        {
            activity: "walking",
            motion: "stair climbing, slow pace",
            met: "4.0",
        },
        {
            activity: "walking",
            motion: "stair climbing, fast pace",
            met: "8.8",
        },
        {
            activity: "walking",
            motion: "using crutches",
            met: "5.0",
        },
        {
            activity: "walking",
            motion: "walking, household",
            met: "2.0",
        },
        {
            activity: "walking",
            motion: "walking, less than 2.0 mph, level, strolling, very slow",
            met: "2.0",
        },
        {
            activity: "walking",
            motion: "walking, 2.0 mph, level, slow pace, firm surface",
            met: "2.8",
        },
        {
            activity: "walking",
            motion: "walking for pleasure",
            met: "3.5",
        },
        {
            activity: "walking",
            motion: "walking from house to car or bus, from car or bus to go places, from car or bus to and from the worksite",
            met: "2.5",
        },
        {
            activity: "walking",
            motion: "walking to neighbor’s house or family’s house for social reasons",
            met: "2.5",
        },
        {
            activity: "walking",
            motion: "walking the dog",
            met: "3.0",
        },
        {
            activity: "walking",
            motion: "walking, 2.5 mph, level, firm surface",
            met: "3.0",
        },
        {
            activity: "walking",
            motion: "walking, 2.5 mph, downhill",
            met: "3.3",
        },
        {
            activity: "walking",
            motion: "walking, 2.8 to 3.2 mph, level, moderate pace, firm surface",
            met: "3.5",
        },
        {
            activity: "walking",
            motion: "walking, 3.5 mph, level, brisk, firm surface, walking for exercise",
            met: "4.3",
        },
        {
            activity: "walking",
            motion: "walking, 2.9 to 3.5 mph, uphill, 1 to 5% grade",
            met: "5.3",
        },
        {
            activity: "walking",
            motion: "walking, 2.9 to 3.5 mph, uphill, 6% to 15% grade",
            met: "8.0",
        },
        {
            activity: "walking",
            motion: "walking, 4.0 mph, level, firm surface, very brisk pace",
            met: "5.0",
        },
        {
            activity: "walking",
            motion: "walking, 4.5 mph, level, firm surface, very, very brisk",
            met: "7.0",
        },
        {
            activity: "walking",
            motion: "walking, 5.0 mph, level, firm surface",
            met: "8.3",
        },
        {
            activity: "walking",
            motion: "walking, 5.0 mph, uphill, 3% grade",
            met: "9.8",
        },
        {
            activity: "walking",
            motion: "walking, for pleasure, work break",
            met: "3.5",
        },
        {
            activity: "walking",
            motion: "walking, grass track",
            met: "4.8",
        },
        {
            activity: "walking",
            motion: "walking, normal pace, plowed field or sand",
            met: "4.5",
        },
        {
            activity: "walking",
            motion: "walking, to work or class",
            met: "4.0",
        },
        {
            activity: "walking",
            motion: "walking, to and from an outhouse",
            met: "2.5",
        },
        {
            activity: "walking",
            motion: "walking, for exercise, 3.5 to 4 mph, with ski poles, Nordic walking, level, moderate pace",
            met: "4.8",
        },
        {
            activity: "walking",
            motion: "walking, for exercise, 5.0 mph, with ski poles, Nordic walking, level, fast pace",
            met: "9.5",
        },
        {
            activity: "walking",
            motion: "walking, for exercise, with ski poles, Nordic walking, uphill",
            met: "6.8",
        },
        {
            activity: "walking",
            motion: "walking, backwards, 3.5 mph, level",
            met: "6.0",
        },
        {
            activity: "walking",
            motion: "walking, backwards, 3.5 mph, uphill, 5% grade",
            met: "8.0",
        },
    ],
    water_activities: [
        {
            activity: "water activities",
            motion: "boating, power, driving",
            met: "2.5",
        },
        {
            activity: "water activities",
            motion: "boating, power, passenger, light",
            met: "1.3",
        },
        {
            activity: "water activities",
            motion: "canoeing, on camping trip",
            met: "4.0",
        },
        {
            activity: "water activities",
            motion: "canoeing, harvesting wild rice, knocking rice off the stalks",
            met: "3.3",
        },
        {
            activity: "water activities",
            motion: "canoeing, portaging",
            met: "7.0",
        },
        {
            activity: "water activities",
            motion: "canoeing, rowing, 2.0-3.9 mph, light effort",
            met: "2.8",
        },
        {
            activity: "water activities",
            motion: "canoeing, rowing, 4.0-5.9 mph, moderate effort",
            met: "5.8",
        },
        {
            activity: "water activities",
            motion: "canoeing, rowing, kayaking, competition, >6 mph, vigorous effort",
            met: "12.5",
        },
        {
            activity: "water activities",
            motion: "canoeing, rowing, for pleasure, general",
            met: "3.5",
        },
        {
            activity: "water activities",
            motion: "canoeing, rowing, in competition, or crew or sculling",
            met: "12.0",
        },
        {
            activity: "water activities",
            motion: "diving, springboard or platform",
            met: "3.0",
        },
        {
            activity: "water activities",
            motion: "kayaking, moderate effort",
            met: "5.0",
        },
        {
            activity: "water activities",
            motion: "paddle boat",
            met: "4.0",
        },
        {
            activity: "water activities",
            motion: "sailing, boat and board sailing, windsurfing, ice sailing, general",
            met: "3.0",
        },
        {
            activity: "water activities",
            motion: "sailing, in competition",
            met: "4.5",
        },
        {
            activity: "water activities",
            motion: "sailing, Sunfish/Laser/Hobby Cat, Keel boats, ocean sailing, yachting, leisure",
            met: "3.3",
        },
        {
            activity: "water activities",
            motion: "skiing, water or wakeboarding",
            met: "6.0",
        },
        {
            activity: "water activities",
            motion: "jet skiing, driving, in water",
            met: "7.0",
        },
        {
            activity: "water activities",
            motion: "skindiving, fast",
            met: "15.8",
        },
        {
            activity: "water activities",
            motion: "skindiving, moderate",
            met: "11.8",
        },
        {
            activity: "water activities",
            motion: "skindiving, scuba diving, general",
            met: "7.0",
        },
        {
            activity: "water activities",
            motion: "snorkeling",
            met: "5.0",
        },
        {
            activity: "water activities",
            motion: "surfing, body or board, general",
            met: "3.0",
        },
        {
            activity: "water activities",
            motion: "surfing, body or board, competitive",
            met: "5.0",
        },
        {
            activity: "water activities",
            motion: "paddle boarding, standing",
            met: "6.0",
        },
        {
            activity: "water activities",
            motion: "swimming laps, freestyle, fast, vigorous effort",
            met: "9.8",
        },
        {
            activity: "water activities",
            motion: "swimming laps, freestyle, front crawl, slow, light or moderate effort",
            met: "5.8",
        },
        {
            activity: "water activities",
            motion: "swimming, backstroke, general, training or competition",
            met: "9.5",
        },
        {
            activity: "water activities",
            motion: "swimming, backstroke, recreational",
            met: "4.8",
        },
        {
            activity: "water activities",
            motion: "swimming, breaststroke, general, training or competition",
            met: "10.3",
        },
        {
            activity: "water activities",
            motion: "swimming, breaststroke, recreational",
            met: "5.3",
        },
        {
            activity: "water activities",
            motion: "swimming, butterfly, general",
            met: "13.8",
        },
        {
            activity: "water activities",
            motion: "swimming, crawl, fast speed, ~75 yards/minute, vigorous effort",
            met: "10.0",
        },
        {
            activity: "water activities",
            motion: "swimming, crawl, medium speed, ~50 yards/minute, vigorous effort",
            met: "8.3",
        },
        {
            activity: "water activities",
            motion: "swimming, lake, ocean, river",
            met: "6.0",
        },
        {
            activity: "water activities",
            motion: "swimming, leisurely, not lap swimming, general",
            met: "6.0",
        },
        {
            activity: "water activities",
            motion: "swimming, sidestroke, general",
            met: "7.0",
        },
        {
            activity: "water activities",
            motion: "swimming, synchronized",
            met: "8.0",
        },
        {
            activity: "water activities",
            motion: "swimming, treading water, fast, vigorous effort",
            met: "9.8",
        },
        {
            activity: "water activities",
            motion: "swimming, treading water, moderate effort, general",
            met: "3.5",
        },
        {
            activity: "water activities",
            motion: "tubing, floating on a river, general",
            met: "2.3",
        },
        {
            activity: "water activities",
            motion: "water aerobics, water calisthenics",
            met: "5.5",
        },
        {
            activity: "water activities",
            motion: "water polo",
            met: "10.0",
        },
        {
            activity: "water activities",
            motion: "water volleyball",
            met: "3.0",
        },
        {
            activity: "water activities",
            motion: "water jogging",
            met: "9.8",
        },
        {
            activity: "water activities",
            motion: "water walking, light effort, slow pace",
            met: "2.5",
        },
        {
            activity: "water activities",
            motion: "water walking, moderate effort, moderate pace",
            met: "4.5",
        },
        {
            activity: "water activities",
            motion: "water walking, vigorous effort, brisk pace",
            met: "6.8",
        },
        {
            activity: "water activities",
            motion: "whitewater rafting, kayaking, or canoeing",
            met: "5.0",
        },
        {
            activity: "water activities",
            motion: "windsurfing, not pumping for speed",
            met: "5.0",
        },
        {
            activity: "water activities",
            motion: "windsurfing or kitesurfing, crossing trial",
            met: "11.0",
        },
        {
            activity: "water activities",
            motion: "windsurfing, competition, pumping for speed",
            met: "13.5",
        },
    ],
    winter: [
        {
            activity: "winter activities",
            motion: "dog sledding, mushing",
            met: "7.5",
        },
        {
            activity: "winter activities",
            motion: "dog sledding, passenger",
            met: "2.5",
        },
        {
            activity: "winter activities",
            motion: "moving ice house, set up/drill holes",
            met: "6.0",
        },
        {
            activity: "winter activities",
            motion: "ice fishing, sitting",
            met: "2.0",
        },
        {
            activity: "winter activities",
            motion: "skating, ice dancing",
            met: "14.0",
        },
        {
            activity: "winter activities",
            motion: "skating, ice, 9 mph or less",
            met: "5.5",
        },
        {
            activity: "winter activities",
            motion: "skating, ice, general",
            met: "7.0",
        },
        {
            activity: "winter activities",
            motion: "skating, ice, rapidly, more than 9 mph, not competitive",
            met: "9.0",
        },
        {
            activity: "winter activities",
            motion: "skating, speed, competitive",
            met: "13.3",
        },
        {
            activity: "winter activities",
            motion: "ski jumping, climb up carrying skis",
            met: "7.0",
        },
        {
            activity: "winter activities",
            motion: "skiing, general",
            met: "7.0",
        },
        {
            activity: "winter activities",
            motion: "skiing, cross country, 2.5 mph, slow or light effort, ski walking",
            met: "6.8",
        },
        {
            activity: "winter activities",
            motion: "skiing, cross country, 4.0-4.9 mph, moderate speed and effort, general",
            met: "9.0",
        },
        {
            activity: "winter activities",
            motion: "skiing, cross country, 5.0-7.9 mph, brisk speed, vigorous effort",
            met: "12.5",
        },
        {
            activity: "winter activities",
            motion: "skiing, cross country, >8.0 mph, elite skier, racing",
            met: "15.0",
        },
        {
            activity: "winter activities",
            motion: "skiing, cross country, hard snow, uphill, maximum, snow mountaineering",
            met: "15.5",
        },
        {
            activity: "winter activities",
            motion: "skiing, cross-country, skating",
            met: "13.3",
        },
        {
            activity: "winter activities",
            motion: "skiing, cross-country, biathlon, skating technique",
            met: "13.5",
        },
        {
            activity: "winter activities",
            motion: "skiing, downhill, alpine or snowboarding, light effort, active time only",
            met: "4.3",
        },
        {
            activity: "winter activities",
            motion: "skiing, downhill, alpine or snowboarding, moderate effort, general, active time only",
            met: "5.3",
        },
        {
            activity: "winter activities",
            motion: "skiing, downhill, vigorous effort, racing",
            met: "8.0",
        },
        {
            activity: "winter activities",
            motion: "skiing, roller, elite racers",
            met: "12.5",
        },
        {
            activity: "winter activities",
            motion: "sledding, tobogganing, bobsledding, luge",
            met: "7.0",
        },
        {
            activity: "winter activities",
            motion: "snow shoeing, moderate effort",
            met: "5.3",
        },
        {
            activity: "winter activities",
            motion: "snow shoeing, vigorous effort",
            met: "10.0",
        },
        {
            activity: "winter activities",
            motion: "snowmobiling, driving, moderate",
            met: "3.5",
        },
        {
            activity: "winter activities",
            motion: "snowmobiling, passenger",
            met: "2.0",
        },
        {
            activity: "winter activities",
            motion: "snow shoveling, by hand, moderate effort",
            met: "5.3",
        },
        {
            activity: "winter activities",
            motion: "snow shoveling, by hand, vigorous effort",
            met: "7.5",
        },
        {
            activity: "winter activities",
            motion: "snow blower, walking and pushing",
            met: "2.5",
        },
    ],
};

export default met;
